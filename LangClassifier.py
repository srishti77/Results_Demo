...
def get_text_classifier_model(text_field, level_label, model_name, pretrained_lang_model_name=None):
    ...
    rnn_learner = text_data.get_model(opt_fn, 500, bptt, em_sz,nh, nl,
                                      dropouti=0.01, dropout=0.01, wdrop=0.01, dropoute=0.02, dropouth=0.01)
    ...
    logging.info(f'                    ... {accuracy_np(*rnn_learner.predict_with_targs())}')
...
file_write= open('./data/recom_pre_suf_1M/output/classify_pred.txt', 'w')
